...
CMainWindow::CMainWindow() : m_cyChar{0},m_cxChar{0}
{
	LPCTSTR strWndClass = AfxRegisterWndClass(
		0, AfxGetApp()->LoadCursorW(IDC_ARROW), (HBRUSH)(COLOR_3DFACE + 1), AfxGetApp()->LoadStandardIcon(IDI_WINLOGO)
	);
...
void CMainWindow::FillListBox()
{
	...
	CClientDC dc(this);
	::EnumFontFamilies((HDC)dc, NULL, (FONTENUMPROC)
		EnumFontFamProc,
		(LPARAM)this);
}
...
int CMainWindow::OnCreate(LPCREATESTRUCT lpcs)
{
	...
	m_wndLBTitle.Create(_T("Typefaces"), WS_CHILD | WS_VISIBLE |
		SS_LEFT,
		rect, this);
	rect.SetRect(m_cxChar * 2, m_cyChar * 2, m_cxChar * 48,
		m_cyChar * 18);
	m_wndListBox.CreateEx(WS_EX_CLIENTEDGE, _T("listbox"), NULL,
		WS_CHILD | WS_VISIBLE | LBS_STANDARD, rect, this,
		IDC_LISTBOX);
	rect.SetRect(m_cxChar * 2, m_cyChar * 19, m_cxChar * 48,
		m_cyChar * 20);
	m_wndCheckBox.Create(_T("Show TrueType fonts only"),
		WS_CHILD |
		WS_VISIBLE | BS_AUTOCHECKBOX, rect, this,
		IDC_CHECKBOX);
	rect.SetRect(m_cxChar * 2, m_cyChar * 21, m_cxChar * 66,
		m_cyChar * 25);
	m_wndGroupBox.Create(_T("Sample"), WS_CHILD | WS_VISIBLE |
		BS_GROUPBOX,
		rect, this, (UINT)-1);
	rect.SetRect(m_cxChar * 4, m_cyChar * 22, m_cxChar * 64,
		(m_cyChar * 99) / 4);
	m_wndSampleText.Create(_T(""), WS_CHILD | WS_VISIBLE |
		SS_CENTER, rect,
		this, IDC_SAMPLE);
	rect.SetRect(m_cxChar * 50, m_cyChar * 2, m_cxChar * 66,
		m_cyChar * 4);
	m_wndPushButton.Create(_T("Print Sample"), WS_CHILD |
		WS_VISIBLE |
