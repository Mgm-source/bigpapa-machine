...
void MaskedBitmap::DrawTransparent(CDC* pDC, int x, int y, COLORREF clrTransparency)
{
	BITMAP bm;
	GetBitmap(&bm);
...
// 
	...
	CBitmap bitmapXor;
	bitmapXor.CreateCompatibleBitmap(&dcImage, bm.bmWidth,
		bm.bmHeight);
	...
	CBitmap bitmapTemp;
	bitmapTemp.CreateCompatibleBitmap(&dcImage, bm.bmWidth,
		bm.bmHeight);
...
void MaskedBitmap::Draw(CDC* pDC, int x, int y)
{
	BITMAP bm;
	GetBitmap(&bm);
	CPoint size(bm.bmWidth, bm.bmHeight);
